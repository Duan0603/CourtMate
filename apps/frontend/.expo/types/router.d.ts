...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(tabs)` | `/(tabs)/chat` | `/(tabs)/dashboard` | `/(tabs)/profile` | `/(tabs)/tracker` | `/(tabs)\_layout` | `/..\src\components\ui\Typography` | `/_sitemap` | `/admin` | `/chat` | `/dashboard` | `/edit-profile` | `/profile` | `/tracker`;
