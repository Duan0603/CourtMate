...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(tabs)` | `/(tabs)/chat` | `/(tabs)/home` | `/(tabs)/profile` | `/(tabs)/tracker` | `/_sitemap` | `/admin` | `/chat` | `/create-tournament` | `/edit-profile` | `/home` | `/organizer` | `/payment/cancel` | `/payment/return` | `/profile` | `/tracker`;
